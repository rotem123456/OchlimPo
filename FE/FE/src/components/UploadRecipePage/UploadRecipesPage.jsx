...
const tags = ["Vegi", "Meat", "Cocktails", "Lunch", "Gluten Free", "Kosher"];
...
const RecipeUpload = () => {
  ...
  const handleImageRemove = () => {
    ...
    setSelectedImage(
      updatedImages.length > 0 ? updatedImages[0].preview : null
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formattedData = {
        ...
        shortDescription: steps.join(' '), 
        foodCategory: mapDifficultyToFoodCategory(difficulty), 
        tags: selectedTags.map(tag => mapTagToFoodType(tag)), 
        ingredients: ingredients
          .filter(ing => ing.name.trim() !== '') 
          .map(ing => ({
            ingridient: mapIngredientNameToEnum(ing.name), 
            amount: parseFloat(ing.amount) || 1
          }))
      };
      
      const response = await fetch(`${BEpath}/recipe/create`, {
        method: "POST",
        headers: { 
      ...
      if (response.ok) {
        ...
        console.error("Server responded with error:", errorData,status);
      }
    } catch (error) {
  ...
  const mapDifficultyToFoodCategory = (difficulty) => {

    return "course"; 
  };
  
  const mapTagToFoodType = (tag) => {
    // Map UI tag values to the FoodType enum values
    const tagMap = {
      "Vegi": "vegitarian",
      "Meat": "meat",
      "Cocktails": "snack",
    ...
    return tagMap[tag] || "comfort"; 
  };
  
  const mapIngredientNameToEnum = (name) => {
    ...
    return ingredientMap[lowerName] || "salt"; 
  };
      ...
      <div className="container">
        ...
        <div className="form-section">
          <form className="recipe-form" onSubmit={handleSubmit}>
            <div className="recipe-name" onClick={() => setIsEditingName(true)}>
              {isEditingName ? (
                <input
                  type="text"
                  value={recipeName}
